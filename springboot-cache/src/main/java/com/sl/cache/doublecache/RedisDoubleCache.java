 ...
 */
@Slf4j
public class RedisDoubleCache extends RedisCache {
    ...
    public RedisDoubleCache(String name, RedisCacheWriter cacheWriter, RedisCacheConfiguration cacheConfig, Map<String, Long> configMap, String secondCachePrefix) {
        ...
        try {
            ...
            if (value != null) {
                ExecutorsUtils.getExecutorSecondCache().execute(() -> {
                    //asyncCache(key,valueLoader);
                    ReentrantLock scondLock = null;
                    try {
                        scondLock = getLock(secondKey);
                        scondLock.lock();
                        //获取一级
                        ValueWrapper res = this.get(key);
                        if (res != null) {
                            log.info("sync cache exist");
                            return;
                        }
                        log.info("sync cache update: {}", key);
                        //执行回调
                        T newValue = valueLoader.call();
                        //刷新一级缓存、刷新二级缓存
                        put(key, newValue);
                    } catch (Exception e) {
                        log.info("sync cache error ",e);
                    } finally {
                        scondLock.unlock();
                        keyLockMap.remove(secondKey);
                    }
                });
            }
        } catch (Exception e) {
    ...
    public <T> ValueWrapper getSecond(Object key, Callable<T> valueLoader) {
        Object value = lookupSecondV2(key, valueLoader);
    ...
    private <T> T asyncCache(Object key, Callable<T> valueLoader) {
        ...
        try {
            ...
            log.error("asyncCache exception ",e);
            throw new ValueRetrievalException(key, valueLoader, e.getCause());
        } finally {
            lock.unlock();
